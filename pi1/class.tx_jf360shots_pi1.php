...
class tx_jf360shots_pi1 extends tslib_pibase
{
	...
	public function parseTemplate($dir='')
	{
		...
		if (T3JQUERY === TRUE) {
			tx_t3jquery::addJqJS();
			if ($this->conf['config.']['wheelable'] && class_exists(t3lib_utility_VersionNumber) && t3lib_utility_VersionNumber::convertVersionNumberToInteger($this->pagerenderer->getExtensionVersion('t3jquery')) <= 1010003) {
